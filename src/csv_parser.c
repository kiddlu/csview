...
static void field_callback(void *s, size_t len, void *data)
{
    ...
    // Expand fields array
    state->current_record->fields = realloc(
        state->current_record->fields, (state->current_record->field_count + 1) * sizeof(char *));
...
int parse_csv(FILE *input, struct csv_data *csv, struct cli_args args)
{
    ...
    if (args.number)
    {
        ...
        csv->column_widths = realloc(csv->column_widths, csv->max_columns * sizeof(int));
        memmove(
            &csv->column_widths[1], &csv->column_widths[0], (csv->max_columns - 1) * sizeof(int));
