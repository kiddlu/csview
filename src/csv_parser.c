...
static void update_column_widths(struct csv_data *csv, struct csv_record *record) {
    for (int i = 0; i < record->field_count; i++) {
        if (i >= csv->max_columns) {
            csv->max_columns = i + 1;
            csv->column_widths = realloc(csv->column_widths, csv->max_columns * sizeof(int));
...
static void field_callback(void *s, size_t len, void *data) {
    ...
    // Expand fields array
    state->current_record->fields = realloc(state->current_record->fields,
                                          (state->current_record->field_count + 1) * sizeof(char *));
...
static void record_callback(int c __attribute__((unused)), void *data) {
    ...
    if (state->csv->record_count >= INITIAL_RECORD_CAPACITY) {
        state->csv->records = realloc(state->csv->records,
                                    (state->csv->record_count + 1) * sizeof(struct csv_record));
    }
...
int parse_csv(FILE *input, struct csv_data *csv, struct cli_args args) {
    ...
    while ((bytes_read = fread(buffer, 1, BUFFER_SIZE, input)) > 0) {
        if (csv_parse(&parser, buffer, bytes_read, field_callback, record_callback, &state) != bytes_read) {
    ...
    if (args.number) {
        ...
        if (seq_width < 1) seq_width = 1;

        // Shift column widths to make room for sequence number
        csv->max_columns++;
        csv->column_widths = realloc(csv->column_widths, csv->max_columns * sizeof(int));
        memmove(&csv->column_widths[1], &csv->column_widths[0],
                (csv->max_columns - 1) * sizeof(int));
