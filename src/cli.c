...
static table_style_t parse_style(const char *style_str) {
    if (strcasecmp(style_str, "none") == 0) return STYLE_NONE;
    if (strcasecmp(style_str, "ascii") == 0) return STYLE_ASCII;
    if (strcasecmp(style_str, "ascii2") == 0) return STYLE_ASCII2;
    if (strcasecmp(style_str, "sharp") == 0) return STYLE_SHARP;
    if (strcasecmp(style_str, "rounded") == 0) return STYLE_ROUNDED;
    if (strcasecmp(style_str, "reinforced") == 0) return STYLE_REINFORCED;
    if (strcasecmp(style_str, "markdown") == 0) return STYLE_MARKDOWN;
    if (strcasecmp(style_str, "grid") == 0) return STYLE_GRID;
...
void print_help(const char *program_name) {
    ...
    printf("      --header-align <ALIGN> Specify the alignment of the table header [default: center]\n");
